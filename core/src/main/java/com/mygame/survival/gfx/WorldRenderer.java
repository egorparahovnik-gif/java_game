...
public final class WorldRenderer {
    ...
    private void renderDayNightAndFireGlow(SpriteBatch batch, ShapeRenderer shapeRenderer, OrthographicCamera camera, GameWorld world, GameAssets assets) {
        ...
        if (world.campfire.health > 0f) {
            ...
            float health01 = MathUtils.clamp(world.campfire.health / GameConstants.CAMPFIRE_MAX_HEALTH, 0f, 1f);
            
            // Night boost increases light intensity in darkness
            float nightBoost = MathUtils.lerp(0.80f, 1.20f, darknessCurve);
            
            // Micro flicker based on fire animation
            float microFlicker = 0.97f
                + 0.03f * MathUtils.sin(world.fireAnimTimeSec * 9.0f)
                + 0.02f * MathUtils.sin(world.fireAnimTimeSec * 16.0f);
            
            // Intensity depends directly on health: full at max health, minimal at low health
            // Mocking effect (subtle flicker) added via microFlicker
            float healthIntensity = MathUtils.lerp(GameConstants.CAMPFIRE_MIN_LIGHT_INTENSITY, GameConstants.CAMPFIRE_MAX_LIGHT_INTENSITY, health01);
            float intensity = healthIntensity * nightBoost * microFlicker;
            intensity = MathUtils.clamp(intensity, 0f, GameConstants.CAMPFIRE_MAX_LIGHT_INTENSITY);

            // Radius depends only on health: shrinks as fire dies, no pulsing
            float healthRadius = MathUtils.lerp(GameConstants.CAMPFIRE_MIN_LIGHT_RADIUS, GameConstants.CAMPFIRE_MAX_LIGHT_RADIUS, health01);
            float drawSize = healthRadius * 2f;
